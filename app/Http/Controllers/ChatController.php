...
class ChatController extends Controller
{
  ...
  private function render(Request $request, ?Conversation $conversation): Response
  {
    ...
    return Inertia::render('chats/index', [
      'conversations' => $conversations->map(
        fn(Conversation $item): array => $this->conversationData($item, $user)
      )->values()
        ->all(),
      'activeConversation' => $activeConversation
        ? $this->conversationData($activeConversation, $user)
        : null,
      'messages' => $activeConversation
        ? $activeConversation->messages
        ->sortBy('created_at')
        ->values()
        ->map(fn(Message $message): array => $this->messageData($message))->all()
        : [],
  ...
  private function conversationQuery(User $user): Builder
  {
    if ($user->hasRole('service_provider')) {
      ...
      return Conversation::whereBelongsTo($providerProfile, 'providerProfile');
    }

    return Conversation::whereBelongsTo($user, 'client');
  }
