...
type Options<T extends object> = Omit<
  ...
  return (
    <div className={cn('w-full', classNames?.wrapper)}>
      ...
      >
        <SelectTrigger
          className={cn(
            'flex h-11! w-full items-center py-2.5',
            SelectIcon ? 'space-x-3' : '',
            className ?? '',
          )}
        >
          <div className={cn('flex items-center gap-2')}>
            {SelectIcon && <SelectIcon className="size-4" />}
...
type MultiSelectOptions<T extends object> = Omit<
...
export function MultiSelect<T extends object>({
  ...
  return (
    <div className={cn('w-full', classNames?.wrapper)}>
      ...
      >
        <ComboboxChips ref={anchor} className={cn('h-10 w-full', className)}>
          <div className="flex w-full items-center gap-1 overflow-x-auto">
            ...
            <ChevronDown className="size-4 stroke-current opacity-30" />
          </div>
        </ComboboxChips>
    ...
    <div className={cn('w-full', classNames?.wrapper)}>
      ...
      >
        <ComboboxInput
          ...
          value={String(value ? getLabel(value) : '')}
          className={className}
        />
        <ComboboxContent>
          <ComboboxEmpty>No items found.</ComboboxEmpty>
